/* $Id: 45675cbed3818a9e1ee3b0c06fddbaaf216e4503 $
 * 
 * @license
 ...
 * {@link DsolUtil}
 * 
 * @version $Id: 45675cbed3818a9e1ee3b0c06fddbaaf216e4503 $
...
public class DsolUtil
{
	...
	{
		return unit == toUnit ? time
				: time.doubleValue() * unit.getValue() / toUnit.getValue();
	}
	...
	public static DateTime getTimeOffset( final SimulatorInterface simulator )
	{
		...
		{
			return new DateTime(
					simulator.getReplication().getTreatment().getStartTime() );
		} catch( final RemoteException e )
		{
			LOG.fatal( "Problem reaching DSOL replication", e );
			throw new NullPointerException( "Replication unreachable" );
	...
	{
		return new Interval( toDateTime( 0.0, treatment ),
				toDateTime( treatment.getWarmupPeriod(), treatment ) );
	}
		...
		final SimulatorInterface simulator, final DateTime time )
	{
		try
		{
			return getSimTime( sourceID, simulator, simTime( time,
					simulator.getReplication().getTreatment() ) );
		} catch( final RemoteException e )
		{
			LOG.fatal( "Problem reaching DSOL replication", e );
			throw new NullPointerException( "Replication unreachable" );
		...
		final SimulatorInterface simulator, final double simTime )
	{
		return new SimTime( // getTimeUnit(simulator),
				sourceID, simTime, getTimeUnit( simulator ),
				getTimeOffset( simulator ).toDate() );
	}
		...
		final DateTimeZone zone )
	{
		try
		{
			return toDateTime( simulator.getSimulatorTime(),
					simulator.getReplication().getTreatment(), zone );
		} catch( final RemoteException e )
		{
			LOG.fatal( "Problem reaching DSOL replication", e );
			throw new NullPointerException( "Replication unreachable" );
