...
public class Logger
{
	...
	public static final EventProducingMap<String, java.util.logging.Logger> LOGGERS = new EventProducingMap<String, java.util.logging.Logger>(
			Collections.synchronizedMap(
					new HashMap<String, java.util.logging.Logger>() ) );
	...
	{
		if( caller instanceof Class ) { return ((Class<?>) caller).getName(); }
	...
	public static synchronized java.util.logging.Logger
		resolveLogger( final Object caller )
	{
	 ...
	 */
	private static synchronized java.util.logging.Logger
		createLogger( final String loggerName )
	{
		...
		try
		{
			handler = (Handler) ClassUtil
					.resolveConstructor( defaultHandler, null ).newInstance();
	...
	public static void severe( final Object caller, final String methodName,
		final String msg )
	{
		Logger.resolveLogger( caller ).logp( Level.SEVERE,
				resolveCaller( caller ), methodName, msg );
	}
	...
	{
		Logger.resolveLogger( caller ).logp( Level.SEVERE,
				resolveCaller( caller ), methodName,
	...
	public static void warning( final Object caller, final String methodName,
		final String msg )
	{
		Logger.resolveLogger( caller ).logp( Level.WARNING,
				resolveCaller( caller ), methodName, msg );
	}
