...
public abstract class GuiceLazyBinderModule extends AbstractModule
{
		...
		final Class<T> serviceTypeImpl )
	{
		...
		{
			/*
			 * final Binding<S> binding = this.owner.getInjector() == null ?
			 * null : this.owner.getInjector().getBinding(serviceType); if
			 * (binding != null) { LOG.warn("Already bound: " +
			 * serviceType.getName() + " to: " +
			 * binding.getKey().getTypeLiteral().getRawType() .getName());
			 * return; }
			 */
					...
					} else
					{
						...
						if( service instanceof AgentStatusObserver )
							owner.getOwnerStatus()
									.subscribe( (AgentStatusObserver) service );
