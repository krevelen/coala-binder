...
public class JsaUtil implements Util
{
	...
	{
		if( INSTANCE == null ) INSTANCE = new JsaUtil();
		return INSTANCE;
	}
	...
	<T extends Node> T getCachedSLNode( final Object node,
		...
		} else if( slNodeType.equals( Node.class ) )
		{
			...
			result = (T) getCachedSLNode( javaObject, new Converter<Node>()
			{
				...
				public Node convert( Object node )
				{
					...
					try
					{
						slNode = SLParser.getParser()
								.parseTerm( node.toString(), true );
					} catch( jade.semantics.lang.sl.parser.ParseException e )
					{
						try
						{
							slNode = SLParser.getParser()
									.parseFormula( node.toString(), true );
			...
			CoalaExceptionFactory.VALUE_NOT_ALLOWED.create( javaObject,
					"Failed to convert JavaType to SL Node of type: "
							+ slNodeType.getSimpleName() );
		return (T) result;
	}
