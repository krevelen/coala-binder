	...
	EveExposingAgent
{
	...
	protected final void updateWrapperStatus( final BasicAgentStatus status )
	{
		...
		EveAgentManager.getInstance().updateWrapperAgentStatus( agentID,
				status );
	}
	...
	public final void onNext( final AgentStatusUpdate update )
	{
		...
		if( this.destroyed ) throw CoalaExceptionFactory.STATUS_UPDATE_FAILED
				.createRuntime( (Agent) null, status,
		...
		if( status.isInitializedStatus() )
		{
			...
			getScheduler().createTask( new JSONRequest( "activate",
					JsonUtil.getJOM().createObjectNode() ), 0 );
		} else if( status.isFinishedStatus() || status.isFailedStatus() )
		{
			...
			getScheduler().createTask( new JSONRequest( "finish",
					JsonUtil.getJOM().createObjectNode() ), 0 );
		}
	}
	...
	public final void onCreate()
	{
		...
		getScheduler().createTask( new JSONRequest( "initialize",
				JsonUtil.getJOM().createObjectNode() ), 0 );
	}
		...
		throws IOException, JSONRPCException
	{
		...
		send( EveUtil.getAddress( payload.getReceiverID() ), "doReceive",
				params );
	}
