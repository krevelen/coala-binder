/* $Id: 2aac609fe3c739e586b7c418d78b98e0f12db7c6 $
 * 
 * @license
...
public class XmlUtil implements Util
{
	...
	public synchronized static <T> XmlContext<T>
		getContextCache( final XmlContextID<T> contextID )
	{
		if( !JAXB_CONTEXT_CACHE.containsKey( contextID ) )
			JAXB_CONTEXT_CACHE.put( contextID, contextID
	 ...
	 * @throws JAXBException
	 */
	public static Unmarshaller
		getUnmarshaller( final XmlContextID<?> contextID ) throws JAXBException
	{
	...
	public static String toString( final XmlContext<?> contextCache,
		final Object xml ) throws JAXBException
	{
	...
	public static void toOutputStream( final XmlContext<?> contextCache,
		final Object xml, final OutputStream os ) throws JAXBException
	{
	...
	public static DatatypeFactory getDatatypeFactory()
	{
		if( _datatypeFactory == null )
		{
			...
			try
			{
				_datatypeFactory = DatatypeFactory.newInstance( dfType,
						Thread.currentThread().getContextClassLoader() );
			} catch( final Exception e )
			{
				throw new Error( "Problem with xml", e );
			}
		}
	 ...
	 * @param elemNames TODO allow wild-cards or xPath 1, 2, etc. ?
	 * @return
	 * @throws Exception
	 */
		...
		final String... elemNames )
	{
		...
		return Observable.merge( resource.getStreams()
				.map( new Func1<ResourceStream, Observable<T>>()
				{
					@Override
				...
				case XMLStreamConstants.START_ELEMENT:
					...
					while( path.equals( elemPath ) )
					{
						final JAXBElement<T> elem = (JAXBElement<T>) context
								.getUnmarshaller()
								.unmarshal( xmlReader, jaxbElementType );
	 ...
	 * @return
	 */
	public static javax.xml.datatype.Duration
		toDuration( final Interval interval )
	{
