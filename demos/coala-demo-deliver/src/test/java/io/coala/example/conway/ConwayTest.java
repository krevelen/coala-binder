...
public class ConwayTest
{
	...
	{
		CellWorldLattice.getGlobalTransitions()
				.subscribe( new Action1<CellStateTransition>()
				{
	...
	public void testBasicMethods() throws Exception
	{
		...
		Assert.assertTrue( "Should be smaller",
				state1.compareTo( state2 ) < 0 );
		Assert.assertTrue( "Should be smaller",
				state2.compareTo( state3a ) < 0 );
		Assert.assertTrue( "Should be smaller",
				state1.compareTo( state3a ) < 0 );
	...
	public void testCellWorld() throws Exception
	{
		...
		final CountDownLatch latch = new CountDownLatch(
				cellStates.size() * cellStates.get( 0 ).size() );
