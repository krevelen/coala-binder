...
public class DeliverTest
{
		...
		final TaskType type, final FleetID... fleetIDs )
	{
		...
		final LocationID locationID = scenario.getLocationDistribution( type )
				.draw();
		final Duration duration = scenario.getDurationDistribution( type )
				.draw();
		final DateTime startTime = scenario.getOffset().plus(
				scenario.getDailyWindowStartOffsetDistribution( type ).draw() );
		final Interval interval = new Interval( startTime, startTime.plus(
				scenario.getWindowDurationDistribution( type ).draw() ) );
		final Collection<Task> result = new HashSet<>();
		for( FleetID fleetID : fleetIDs )
			result.add(
					new Task( fleetID, type, locationID, duration, interval ) );
		return result;
	}
	...
	public void testTrigionScenario() throws Exception
	{
		...
		final Observable<Task> incidents = Observable
				.from( observableIncidents );
		...
		/*
		 * - [optional] register replication company {@link
		 * com.deal.planner.server.rest2.CompanyResource#createCompany(String,
		 * String)}
		 * https://docs.google.com/a/almende.org/document/d/19XQcC4qZsHkx-
		 * UMZs6dSMh7Htday03orPXUaH2Lq9ZQ/edit#heading=h.k1huhio84ef3
		 */
		...
		/*
		 * - [optional] register replication user (i.e. coordinator view
		 * account)
		 * https://docs.google.com/a/almende.org/document/d/19XQcC4qZsHkx-
		 * UMZs6dSMh7Htday03orPXUaH2Lq9ZQ/edit#heading=h.d90ga1fk4cg6
		 */
		...
		/*
		 * - [optional] register replication's (external) vehicle agents to
		 * listen for order status update events a. either: Configure the
		 * mapping on logged in user dealrootuser@gmail.com {@link
		 * com.deal.planner.shared.twig.bean.DefaultUser#DEFAULT_DEAL} b. or:
		 * for initial setup, add to {@link
		 * MyUserSetup#orderUpdatedStatusUpdatedEventHandlers} list:
		 * EventHandler externalAgentHandler = new EventHandler(
		 * "External Agent Name", "http://127.0.0.1/....", false,
		 * AgentType.EXTERNAL, "Description..");
		 ...
		 * https://docs.google.com/a/almende.org/document/d/
		 * 1G4cpPGCGAvERH3rs99oxSYpFLwCYz2absVVKv_syhIk/edit#heading=h.
		 * k1huhio84ef3
		 */
		...
		for( Collection<Vehicle> fleet : fleets.values() )
			for( Vehicle vehicle : fleet )
			{
				 ...
				 * https://docs.google.com/a/almende.org/document/d/
				 * 1JWw8zaI_YnCd8VLkUvSH9vnYrms5dywyGy4rMFkzjU/edit#heading=h.
				 * oadgav2n7z6j
				 */
			}
		 ...
		 * https://docs.google.com/a/almende.org/document/d/
		 * 1zD6q1fnBxLBBFULh_ml_HJyQ_rKkrbihpQ_jOWE0Ouk/edit#heading=h.
		 * k1huhio84ef3
		 */
		...
		for( Task task : tasks )
		{
			...
			/*
			 * - [optional] update order status use "status" as the key of the
			 * field, and one of the {@link
			 * com.deal.planner.shared.twig.bean#OrderState} possible values
			 * https://docs.google.com/a/almende.org/document/d/
			 * 1zD6q1fnBxLBBFULh_ml_HJyQ_rKkrbihpQ_jOWE0Ouk/edit#heading=h.
			 * j1xtil2mjsh2
			 */
			...
			/*
			 * - [optional] pull vehicle schedule (or 404 NOT FOUND if not
			 * exists yet) https://docs.google.com/a/almende.org/document/d/
			 * 1af7EIKfn9mM58erujdG6X3awiQ8t0nAsKrfEn5mPCZ0/edit#heading=h.
			 * k1huhio84ef3
			 */
			 ...
			 * https://docs.google.com/a/almende.org/document/d/
			 * 1JWw8zaI_YnCd8VLkUvS_H9vnYrms5dywyGy4rMFkzjU/edit#heading=h.
			 * gdczggmnzrda
			 */
		...
		for( Task task : tasks )
		{
			...
			/*
			 * - [optional] generate truck movement event (push vehicle location
			 * updates) Use REST API at {@link
			 * com.deal.planner.server.rest2.SimulatorResource}
			 */
		...
		/*
		 * 4. repeat: generate incident orders + trigger/notify ACO reschedule
		 * with designated (sub)fleet
		 */
		...
		incidents.subscribe( new Observer<Task>()
		{
			...
			public void onNext( final Task incident )
			{
				...
				/*
				 * - [optional] generate truck movement event (push vehicle
				 * location updates) Use REST API at {@link
				 * com.deal.planner.server.rest2.SimulatorResource}
				 */
		...
		/*
		 * 5. Analyze/export logs - GET
		 * /rest2/logs/entities/trips/truck/<truck_key> - GET
		 * /rest2/logs/entities/trips/order/<order_key>
		 */
		...
		for( Collection<Vehicle> fleet : fleets.values() )
			for( Vehicle vehicle : fleet )
				DealUtil.deleteVehicle(
						dealKeyCache.get( vehicle.id.getValue() ) );

		/*
		 * 6. cleanup: destroy replication orders, vehicles, user, company -
		 * delete Order: DELETE to /rest2/entity/order/123 - delete Vehicle:
		 * DELETE to /rest2/entity/truck/123
		 */

		/*
		 * NOTE: All the Company's users MUST be deleted first before deleting
		 * the Company - [optional] delete User: DELETE to
		 * /rest2/users/email@example.com - [optional] delete Company: DELETE to
		 * /rest2/companies/123
		 */
