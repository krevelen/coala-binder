...
public class DynamicBeanFactory
{
	...
	public static <T extends DynamicBean> T
		cloneDynamicBean( T pClonedDynamicBean )
	{
	...
	public static <T extends DynamicBean> T
		parseDynamicBean( DynamicBean pDynamicBean, Class<T> pDynamicBeanClass )
	{
		...
		Class<T> pDynamicBeanClass )
	{
		try
		{
			...
			{
				return (T) pDynamicBeanClass.newInstance()
						.useJSONString( pJSONString );
			}
		} catch( InstantiationException ex )
		{
			LOG.warn( "For bean class " + pDynamicBeanClass.getName()
		...
		String pDeserializedString, Class<T> pDynamicBeanClass )
	{
		try
		{
			...
			if( !tReturnBean
					.hasCustomDeserializer() ) { throw new IllegalArgumentException(
		...
		Map<String, String> pStringHashMap, Class<T> pDynamicBeanClass )
	{
		...
		{
			return (T) pDynamicBeanClass.newInstance()
					.useStringHashMap( pStringHashMap );
		} catch( Exception ex )
		{
